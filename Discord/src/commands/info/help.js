...
module.exports = class HelpCommand extends Command {
  ...
  run(message, args) {
    ...
    let disabledCommands =
      message.client.db.settings.selectDisabledCommands.pluck().get(message.guild.id) || [];
    ...
    if (
      ...
      if (command.aliases)
        embed.addField("Aliases", command.aliases.map((c) => `\`${c}\``).join(" "));
      if (command.examples)
        embed.addField("Examples", command.examples.map((c) => `\`${prefix}${c}\``).join("\n"));
      ...
      message.client.commands.forEach((command) => {
        if (!disabledCommands.includes(command.name)) {
          if (
            ...
            command.userPermissions.every((p) => message.member.hasPermission(p)) &&
            !all &&
            !command.hidden
          )
      ...
      const total =
        Object.values(commands).reduce((a, b) => a + b.length, 0) - commands[OWNER].length;
      const size = message.client.commands.size - commands[OWNER].length;

      embed // Build help embed
        .setTitle("HentaiWeb's Commands")
        .setDescription(
          stripIndent`
          **Prefix:** \`${prefix}\`
