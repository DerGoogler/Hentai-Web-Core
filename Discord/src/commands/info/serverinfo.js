...
module.exports = class ServerInfoCommand extends Command {
  ...
  run(message) {
    ...
    const embed = new MessageEmbed()
      ...
      .addField(
        "System Channel",
        message.guild.systemChannel ? `${message.guild.systemChannel}` : "`None`",
        true
      )
      .addField(
        "AFK Channel",
        message.guild.afkChannel ? `${voice} ${message.guild.afkChannel.name}` : "`None`",
        true
      )
      .addField(
        "AFK Timeout",
        message.guild.afkChannel
          ? `\`${moment.duration(message.guild.afkTimeout * 1000).asMinutes()} minutes\``
          : "`None`",
        true
      )
      .addField(
