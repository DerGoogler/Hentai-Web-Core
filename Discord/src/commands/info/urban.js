...
module.exports = class UrbanCommand extends Command {
  ...
  async run(message, args) {
    ...
    if (!message.channel.nsfw)
      return this.sendErrorMessage(message, 0, "Please try this command again in a NSFW channel");

    const query = encodeURIComponent(args.join(" ")); //encode the args to fetch info from urban dictionary

    const { list } = await fetch(`https://api.urbandictionary.com/v0/define?term=${query}`).then(
      (response) => response.json()
    );

    if (!list.length)
      return this.sendErrorMessage(message, 0, `No results found for ${args.join(" ")}.`);
