...
module.exports = class WarnPurgeCommand extends Command {
  ...
  async run(message, args) {
    const member =
      this.getMemberFromMention(message, args[0]) || message.guild.members.cache.get(args[0]);
    if (!member)
    ...
    // Warn
    let warns = message.client.db.users.selectWarns.pluck().get(member.id, message.guild.id) || {
      warns: [],
    };
    ...
    // Purge
    const messages = (await message.channel.messages.fetch({ limit: amount })).filter(
    ...
    if (autoKick && warns.warns.length === autoKick) {
      message.client.commands
        ...
        .run(message, [
          member.id,
          `Warn limit reached. Automatically kicked by ${message.guild.me}.`,
        ]);
    }
  }
};
