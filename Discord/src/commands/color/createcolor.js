...
module.exports = class CreateColorCommand extends Command {
  constructor(client) {
    super(client, {
      ...
      description:
        "Creates a new role for the given color hex. Color roles are denoted by the prefix `#`.",
  ...
  async run(message, args) {
    let hex = args.shift();
    if (!rgx.test(hex))
      return this.sendErrorMessage(message, 0, "Please provide a valid color hex and color name");
