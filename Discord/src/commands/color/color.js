...
module.exports = class ColorCommand extends Command {
  ...
  async run(message, args) {
    ...
    const oldColor =
      message.member.roles.color && message.member.roles.color.name.startsWith("#")
    ...
    const role =
      this.getRoleFromMention(message, args[0]) || message.guild.roles.cache.get(args[0]);
    let color;
    if (role && colors.get(role.id)) color = role;
    if (!color) {
      color = colors.find((c) => {
        return (
          colorName == c.name.slice(1).toLowerCase().replace(/\s/g, "") ||
    ...
    if (!color)
      return this.sendErrorMessage(
        message,
        0,
        `Please provide a valid color, use ${prefix}colors for a list`
      );
    // Color exists
    else {
      try {
