...
module.exports = class SetStarboardChannelCommand extends Command {
  ...
  run(message, args) {
    const starboardChannelId = message.client.db.settings.selectStarboardChannelId
      .pluck()
    ...
    if (args.length === 0) {
      ...
      return message.channel.send(
        embed.addField("Starboard Channel", `${oldStarboardChannel} ➔ \`None\``)
      );
    }
    ...
    message.channel.send(
      embed.addField("Starboard Channel", `${oldStarboardChannel} ➔ ${starboardChannel}`)
    );
  }
};
