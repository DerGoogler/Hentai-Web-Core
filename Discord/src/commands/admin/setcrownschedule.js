...
module.exports = class SetCrownScheduleCommand extends Command {
  constructor(client) {
    super(client, {
      ...
      description:
        stripIndent`
        Sets the schedule for HentaiWeb's crown role rotation. 
  ...
  run(message, args) {
    ...
    if (!message.content.includes(" ")) {
      ...
      return message.channel.send(
        embed
          .spliceFields(2, 0, {
            name: "Schedule",
            value: `\`${oldCrownSchedule || "None"}\` ➔ \`None\``,
            inline: true,
          })
    ...
    let crownSchedule = message.content.slice(
    ...
    if (cron.length === 6 && cron[5] != "0") {
      if (description.includes("\n"))
        description =
          description.slice(0, -1) + `, and seconds were changed from \`${cron[5]}\` to \`0\`.`;
      else
        description =
