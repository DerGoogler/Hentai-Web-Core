...
module.exports = class SetVerificationMessageCommand extends Command {
  constructor(client) {
    super(client, {
      ...
      description: oneLine`
        Sets the message HentaiWeb will post in the \`verification channel\`.
      ...
      examples: [
        "setverificationmessage Please read the server rules, then react to this message.",
      ],
    });
  }
  async run(message, args) {
    ...
    if (!args[0]) {
      ...
      const statusUpdate =
        oldStatus != status ? `\`${oldStatus}\` ➔ \`${status}\`` : `\`${oldStatus}\``;

      return message.channel.send(
        embed.addField("Status", statusUpdate, true).addField("Message", "`None`")
      );
    }

    let verificationMessage = message.content.slice(
    ...
    message.channel.send(
      embed.addField("Status", statusUpdate, true).addField("Message", verificationMessage)
    );
