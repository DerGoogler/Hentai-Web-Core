...
module.exports = class SetModChannelsCommand extends Command {
  constructor(client) {
    super(client, {
      ...
      description: oneLine`
        ...
        and HentaiWeb will only respond to members with permission to use those commands.
  ...
  run(message, args) {
    ...
    for (const arg of args) {
      const channel =
        this.getChannelFromMention(message, arg) || message.guild.channels.cache.get(arg);
    ...
    message.channel.send(
      embed.addField("Mod Channels", `${oldModChannels} ➔ ${trimArray(channels).join(" ")}`)
    );
  }
};
