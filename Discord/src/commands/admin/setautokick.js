...
module.exports = class SetAutoKickCommand extends Command {
  constructor(client) {
    super(client, {
      ...
      description: oneLine`
        Sets the amount of warns needed before HentaiWeb will automatically kick someone from your server.
  ...
  run(message, args) {
    const autoKick =
      message.client.db.settings.selectAutoKick.pluck().get(message.guild.id) || "disabled";
    const amount = args[0];
