...
module.exports = class SetMessageDeleteLogCommand extends Command {
  ...
  run(message, args) {
    const messageDeleteLogId = message.client.db.settings.selectMessageDeleteLogId
      .pluck()
    ...
    if (args.length === 0) {
      ...
      return message.channel.send(
        embed.addField("Message Delete Log", `${oldMessageDeleteLog} ➔ \`None\``)
      );
    }
    ...
    message.channel.send(
      embed.addField("Message Delete Log", `${oldMessageDeleteLog} ➔ ${messageDeleteLog}`)
    );
  }
};
