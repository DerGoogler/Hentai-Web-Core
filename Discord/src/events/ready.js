module.exports = async (client) => {
  const activities = [
    { name: "Hentai Web", type: "LISTENING" },
    { name: "@Der_Googler", type: "LISTENING" },
  ];
  ...
  for (const guild of leftGuilds) {
    ...
    client.logger.info(`HentaiWeb has left ${guild.guild_name}`);
  }

  client.logger.info("HentaiWeb is now online");
  client.logger.info(`HentaiWeb is running on ${client.guilds.cache.size} server(s)`);
};
