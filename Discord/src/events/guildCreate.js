...
module.exports = async (client, guild) => {
  client.logger.info(`HentaiWeb has joined ${guild.name}`);
  const serverLog = client.channels.cache.get(client.serverLogId);
  if (serverLog)
    serverLog.send(
      new MessageEmbed().setDescription(`${client.user} has joined **${guild.name}** ${success}`)
    );
  ...
  const adminRole = guild.roles.cache.find(
    (r) => r.name.toLowerCase() === "admin" || r.name.toLowerCase() === "administrator"
  );
  const modRole = guild.roles.cache.find(
    (r) => r.name.toLowerCase() === "mod" || r.name.toLowerCase() === "moderator"
  );
  ...
  // Self-assign color
  try {
    const HentaiWebColor = guild.roles.cache.find((r) => r.name === "#Seagrass");
    if (HentaiWebColor) await guild.me.roles.add(HentaiWebColor);
  } catch (err) {
    client.logger.error(err.message);
  }
};
