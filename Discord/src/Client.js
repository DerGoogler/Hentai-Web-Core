...
class Client extends Discord.Client {
  ...
  constructor(config, options = {}) {
    ...
    /**
     * HentaiWeb's owner ID
     * @type {string}
     */
    this.ownerId = config.ownerId;

    /**
     * HentaiWeb's bug report channel ID
     * @type {string}
     */
    this.bugReportChannelId = config.bugReportChannelId;

    /**
     * HentaiWeb's feedback channel ID
     * @type {string}
     */
    this.feedbackChannelId = config.feedbackChannelId;

    /**
     * HentaiWeb's server log channel ID
     * @type {string}
     */
  ...
  loadCommands(path) {
    ...
    readdirSync(path)
      ...
      .forEach((dir) => {
        const commands = readdirSync(resolve(__basedir, join(path, dir))).filter((f) =>
          f.endsWith("js")
        );
        commands.forEach((f) => {
